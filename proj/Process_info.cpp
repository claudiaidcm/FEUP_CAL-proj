...
Info::Info(string f1, string f2, string f3) {
	graphOriginal = getGraphFromFiles(f1, f2, f3, roads);
}

void Info::displayRoads(){
	cout << roads.size() << " roads:" << endl;
	for (auto road : roads)
		cout << *road << endl;
}

void Info::randomizeRoads(int nIntersections){
...
void Info::preProcessGraph() {
	graph = graphOriginal->preProcessGraph();
}

bool Info::addTruck(Truck * t) {
...
bool Info::addStation(TreatmentStation * s) {
...
bool Info::addContainer(Container * c) {
...
vector<string> Info::getContainerIds() {
	vector<string> res;
...
vector<Vertex<Node> * > Info::generateSingleTruckSingleContainer(Truck * t, Container * c) {
...
vector<Vertex<Node>*> Info::generateOneTruckMultipleContainers(Truck * t, Station * s) {
	vector<Vertex<Node> * > result, temp;
	//assumes that floydWarshallShortestPath was called once before this function
...
vector<vector<Vertex<Node>*>> Info::generateMultipleTrucskMultipleContainers(Station * s, double trucksVolume) {
...
void Info::floydWarshallShortestPath() {
...
vector<Vertex<Node>*> Info::backPreProcess(vector<Vertex<Node>*> shortenedPath) {
...
void Info::recalculateDistContainers(int currentVertexPos, bool isSort) {
