...
public:
	...
	bool addEdgePair(pair<Vertex<T> *, Vertex<T> *> sourceAndDest, double w, bool bothWays = false);//updated to receive vertex pair
...
pair<Vertex<T>*, Vertex<T>*> Graph<T>::getTwoVertexs(const T & sourc, const T & dest) {
	...
	typename vector<Vertex<T>*>::iterator it2 = vertexSet.end();
	int found = 0;
	Vertex<T> *vS = nullptr, *vD = nullptr;
	while (found != 2 && it1 != it2) {
		if ((*it1)->info == sourc)
			{vS=*it1; found++;}
		if ((*it1)->info == dest)
			{vD=*it1; found++;}
...
template<class T>
bool Graph<T>::addEdgePair(pair<Vertex<T>*, Vertex<T>*> sourceAndDest, double w, bool bothWays) {
